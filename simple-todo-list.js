const todo = {  
    action(e) {const target = e.target;
    ...
    add() {const elemText = document.querySelector('.todo__text');
        if (elemText.disabled || !elemText.value.length) {
        return;
        }
        document.querySelector('.todo__items').insertAdjacentHTML('beforeend', this.create(elemText.value));
        elemText.value = '';},
    create(text) {return `<li class="todo__item" data-todo-state="active">
    ...
    init() { const fromStorage = localStorage.getItem('todo');
        if (fromStorage) {
    ...
    update() {const option = document.querySelector('.todo__options').value;
